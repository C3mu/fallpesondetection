...
from customlib import settings as config
#share lib ???
...
class server:
   ...
   def __init__(self, port = 80,pointer=None):#pointer is data base
        """ Constructor """
        #global config.POWERSYS
        global json 
        pwe = ""
        bls = ""
        for i in config.POWERSYS:
            if i.retn() == True:
                pwe += str(1)+","
            else:
                pwe += str(0)+","
        ...
        self.port = port
        self.www_dir = 'sever' # Directory where webpage files are stored

   def activate_server(self):
       t = Thread(target=self.activate, args=())
       t.daemon = True
       t.start()
   ...
   def _wait_for_connections(self):
        ...
        while True:
            ...
            if (request_method == 'GET') | (request_method == 'HEAD'):
                ...
                if (file_requested == '/json_array.json'):
                    #global config.POWERSYS
                    global json 
                    pwe = ""
                    bls = ""
                    ...
                    server_response +=  json.encode()
                    conn.send(server_response)
                    conn.close()
                    continue
                try:
                    file_requested.index('/changestage')
                    pwe = ""
                    for i in config.POWERSYS:
                        pwe += str(i)+","
                    pwe +="0" 
                    json = '{"button":["one","two","three","fourt","FIVE"],\n"stage":['+pwe+']}'
                    b,n = file_requested.split("=")
                    n=int(n)
                    print(n) #change b
                    if config.POWERSYS[n] == 1:
                        config.POWERSYS[n] = 0
                    else:
                        config.POWERSYS[n] = 1
                    response_headers = self._gen_headers(200)
